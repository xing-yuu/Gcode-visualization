	...
	vector<Vec3> vertexs, vector<v_int> faces)
{
	...
	{
		origin_vertexs.push_back(vertexs.at(i));
	}
...
void General_Mesh::insert(vector<Vec3> vertexs, vector<v_int> faces)
{
	...
	int origin_vertexs_size = result_vertex.size();
	int maxsize = result_vertex.max_size();
	for (int i = 0; i < vertexs.size(); i++)
	{
		result_vertex.push_back(vertexs.at(i));
	}
	...
	for (int i = 0; i < faces.size(); i++)
	{
		...
		result_faces.push_back(v_temp);
	}
}

void General_Mesh::genResultMesh(std::string filename)
{
	ofstream fout;
	fout.open(filename);
	for (int i = 0; i < result_vertex.size(); i++)
	{
		fout << "v " << result_vertex[i].m_x << " " << result_vertex[i].m_y << " " << result_vertex[i].m_z << " " << result_color.at(i).m_x << " " << result_color.at(i).m_y << " " << result_color.at(i).m_z << endl;
	}
	//for (int i = 0; i < 376; i++)
...
void General_Mesh::genResultMesh_Nocolor(std::string filename)
{
	ofstream fout;
	fout.open(filename);
	for (int i = 0; i < result_vertex.size(); i++)
